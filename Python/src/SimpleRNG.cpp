...
int SimpleRNG::GetPoisson(double lamb)
{
    return (lamb < 30.0) ? PoissonSmall(lamb) : PoissonLarge(lamb);
}

int SimpleRNG::PoissonSmall(double lamb)
{
    // Algorithm due to Donald Knuth, 1969.
    double p = 1.0, L = exp(-lamb);
	int k = 0;
	do
	{
		k++;
...
int SimpleRNG::PoissonLarge(double lamb)
{
	...
	double c = 0.767 - 3.36/lamb;
	double beta = PI/sqrt(3.0*lamb);
	double alpha = beta*lamb;
	double k = log(c) - lamb - log(beta);
	...
	for(;;)
	{
		...
		double rhs = k + n*log(lamb) - LogFactorial(n);
		if (lhs <= rhs)
			return n;
	}
}
